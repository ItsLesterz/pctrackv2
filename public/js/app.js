...
function renderAlerts() {
  ...
  if (overdue.length) {
    html += `<div class="alert-strip overdue"><i class="bi bi-exclamation-octagon-fill"></i><div><strong>${overdue.length} equipo(s) con mantenimiento vencido:</strong> ${overdue.map(d=>`<span class="mono">${esc(d.name)}</span>`).join(', ')}</div></div>`;
  }

  if (soon.length) {
    html += `<div class="alert-strip soon"><i class="bi bi-bell-fill"></i><div><strong>${soon.length} equipo(s) próximos (≤30 días):</strong> ${soon.map(d=>`<span class="mono">${esc(d.name)}</span>`).join(', ')}</div></div>`;
  }
...
function clientCell(d) {
  ...
  if (!d.client_name && !d.client_phone && !d.client_email) {
    return '<span class="date-mono">—</span>';
  }
...
function openAddModal() {
  ...
  ['editId','fName','fDesc','fLocation','fClientName','fClientPhone','fClientEmail'].forEach(id => {
...
async function saveDevice() {
  ...
  if (!name) {
    toast('El nombre del equipo es requerido.', 'warning');
...
async function confirmDelete() {
  const d = devices.find(x => x.id === deleteId);
  if (!d) return;
...
function openDetail(id) {
  ...
  document.getElementById('detailFooter').innerHTML = `
    ...
    <button class="btn-ghost" onclick="generateServicePDF('${d.id}')"><i class="bi bi-file-earmark-pdf"></i> Generar PDF</button>
...
async function openLogModal() {
  ...
  try {
    ...
    if (!logs.length) {
      body.innerHTML = `<div class="empty-state"><i class="bi bi-clock-history empty-icon"></i><p>No hay actividad registrada.</p></div>`;
      return;
    }
    ...
    body.innerHTML = `<div class="empty-state"><p style="color:var(--red-h)">${esc(e.message)}</p></div>`;
  }
}
...
function generateServicePDF(deviceId) {
  const d = devices.find(x => x.id === deviceId);

  if (!d) {
    toast('No se encontró el equipo.', 'danger');
    return;
  }
  ...
  const next = d.last_maint ? addMonths(d.last_maint, months) : null;
  const as = alertStatus(d);
  const serviceCount = parseInt(d.service_count || 0) || 0;
  const loyaltyLabel = serviceCount >= 3 ? 'Cliente frecuente' : 'Cliente nuevo';

  let statusText = 'Pendiente';
  if (as === 'overdue') statusText = 'Mantenimiento vencido';
  else if (as === 'soon') statusText = 'Próximo a mantenimiento';
  else if (as === 'ok') statusText = 'Al día';

  const serviceDescription = `
Servicio técnico realizado al equipo indicado. Se entrega este comprobante como constancia
del mantenimiento, revisión o reparación efectuada.
  `;
  ...
  const html = `
    ...
    <head>
      ...
      <title>Comprobante de servicio - ${esc(d.name)}</title>
      <style>
        * { box-sizing: border-box; }
        body { font-family: Arial, sans-serif; margin: 0; padding: 30px; color: #111827; background: #ffffff; }
        .actions { max-width: 800px; margin: 0 auto 18px; text-align: right; }
        .btn { background: #111827; color: white; border: none; padding: 10px 16px; border-radius: 8px; cursor: pointer; font-size: 14px; }
        .receipt { max-width: 800px; margin: 0 auto; border: 1px solid #d1d5db; border-radius: 12px; overflow: hidden; }
        .header { background: #111827; color: white; padding: 24px 28px; display: flex; justify-content: space-between; gap: 20px; align-items: center; }
        .brand h1 { margin: 0; font-size: 28px; letter-spacing: -0.5px; }
        .brand p { margin: 4px 0 0; color: #d1d5db; font-size: 13px; }
        .receipt-meta { text-align: right; font-size: 13px; color: #e5e7eb; line-height: 1.5; }
        .body { padding: 28px; }
        .section { margin-bottom: 24px; }
        .section-title { font-size: 15px; font-weight: bold; color: #111827; border-bottom: 2px solid #111827; padding-bottom: 6px; margin-bottom: 12px; text-transform: uppercase; }
        .grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px 24px; }
        .field { margin-bottom: 8px; }
        .label { font-size: 12px; color: #6b7280; text-transform: uppercase; margin-bottom: 3px; }
        .value { font-size: 15px; font-weight: 600; color: #111827; word-break: break-word; }
        .badge { display: inline-block; padding: 5px 10px; border-radius: 999px; background: #ecfdf5; color: #047857; font-size: 12px; font-weight: bold; }
        .service-box { background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 10px; padding: 16px; line-height: 1.5; font-size: 14px; white-space: pre-line; }
        .signatures { display: grid; grid-template-columns: 1fr 1fr; gap: 50px; margin-top: 60px; }
        .signature { text-align: center; font-size: 13px; color: #374151; }
        .line { border-top: 1px solid #111827; margin-bottom: 8px; }
        .footer { background: #f3f4f6; padding: 16px 28px; font-size: 12px; color: #4b5563; text-align: center; }
        @media print {
          body { padding: 0; }
    ...
    <body>
      ...
      <div class="receipt">
        <div class="header">
          <div class="brand">
            <h1>PCTrack</h1>
            <p>Comprobante de servicio técnico</p>
          </div>
          <div class="receipt-meta">
            <div><strong>Fecha:</strong> ${todayLabel}</div>
            <div><strong>ID Equipo:</strong> ${esc(d.id)}</div>
          </div>
        </div>
        ...
        <div class="body">
          <div class="section">
            ...
            <div class="grid">
              <div class="field"><div class="label">Cliente</div><div class="value">${esc(d.client_name || 'No especificado')}</div></div>
              <div class="field"><div class="label">Teléfono / WhatsApp</div><div class="value">${esc(d.client_phone || 'No especificado')}</div></div>
              <div class="field"><div class="label">Correo</div><div class="value">${esc(d.client_email || 'No especificado')}</div></div>
              <div class="field"><div class="label">Tipo de cliente</div><div class="value"><span class="badge">${loyaltyLabel}</span></div></div>
            </div>
          </div>
          ...
          <div class="section">
            ...
            <div class="grid">
              <div class="field"><div class="label">Equipo</div><div class="value">${esc(d.name || 'No especificado')}</div></div>
              <div class="field"><div class="label">Ubicación</div><div class="value">${esc(d.location || 'No especificado')}</div></div>
              <div class="field"><div class="label">Último mantenimiento</div><div class="value">${fmtDate(d.last_maint)}</div></div>
              <div class="field"><div class="label">Próximo mantenimiento</div><div class="value">${fmtDate(next)}</div></div>
              <div class="field"><div class="label">Estado</div><div class="value">${statusText}</div></div>
              <div class="field"><div class="label">Servicios realizados</div><div class="value">${serviceCount}</div></div>
            </div>
          </div>

          <div class="section">
            <div class="section-title">Descripción del servicio</div>
            <div class="service-box">${serviceDescription}

<strong>Notas del equipo:</strong>
${esc(d.description || 'Sin notas adicionales.')}</div>
          </div>

          <div class="section">
            <div class="section-title">Recomendación</div>
            <div class="service-box">Se recomienda realizar mantenimiento preventivo de forma periódica para mantener el buen funcionamiento del equipo, evitar sobrecalentamiento, pérdida de rendimiento y posibles fallos futuros.</div>
          </div>

          <div class="signatures">
            <div class="signature"><div class="line"></div>Firma del técnico</div>
            <div class="signature"><div class="line"></div>Firma del cliente</div>
          </div>
        </div>

        <div class="footer">Documento generado por PCTrack. Este comprobante sirve como respaldo del servicio realizado.</div>
      </div>
    </body>
    </html>
  `;
  ...
  if (!win) {
    toast('El navegador bloqueó la ventana emergente.', 'danger');
    return;
  }
